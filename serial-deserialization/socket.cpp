...
data_packet create_data_packet() {
	...
	std::cout << "Enter a packet name: ";
	std::cin >> my_packet.name;
	std::cout << "What is the ID of the packet? ";
	std::cin >> my_packet.id;
	std::cout << "What is the tail of the packet? ";
	std::cin >> my_packet.tail;
	std::cout << "What is the size of the packet? ";
	std::cin >> my_packet.size;
...
serialized_packet data_packet_serializer(data_packet my_packet) {
	...
	char* my_serialized_packet = new char[16 * sizeof(char) + 1 * sizeof(int) +
		1 * sizeof(int) + my_packet.size * sizeof(int) + 1 * sizeof(int)];
	char* auxptr = my_serialized_packet;


	memcpy(auxptr, my_packet.name, 16 * sizeof(char));
	auxptr += 16 * sizeof(char);
	memcpy(auxptr, &my_packet.id, 1 * sizeof(int));
	auxptr += 1 * sizeof(int);
	memcpy(auxptr, &my_packet.size, 1 * sizeof(int));
	auxptr += 1 * sizeof(int);
	memcpy(auxptr, &my_packet.numbers, my_packet.size * sizeof(int));
	auxptr += my_packet.size * sizeof(int);
	memcpy(auxptr, &my_packet.tail, 1 * sizeof(int));

	serialized_packet output;
	output.data = my_serialized_packet;
	output.length = 16 * sizeof(char) + 1 * sizeof(int) + 1 * sizeof(int) + my_packet.size * sizeof(int) + 1 * sizeof(int);
	return output;
}
...
data_packet packet_deserializer(char* serialized_packet) {
	...
	auxptr += deserialized_packet.size * sizeof(int);
...
int Server_TCP::send_packet(data_packet message) {
...
int Server_TCP::receive_packet(data_packet& my_packet) {
	char RxBuffer[MAX_BUFFER_SIZE] = {};
	memset(RxBuffer, 0, MAX_BUFFER_SIZE);
	int num_bytes = recv(this->client_socket, RxBuffer, MAX_BUFFER_SIZE, 0);
...
int Client_TCP::send_packet(data_packet message) {
	struct serialized_packet serialized = packet_serializer(message);
	return send(this->active_socket, serialized.data, serialized.length, 0);
}
...
int Client_TCP::receive_packet(data_packet& my_packet) {
	char RxBuffer[MAX_BUFFER_SIZE] = {};
	memset(RxBuffer, 0, MAX_BUFFER_SIZE);
	int num_bytes = recv(this->active_socket, RxBuffer, MAX_BUFFER_SIZE, 0);
	my_packet = packet_deserializer(RxBuffer);
